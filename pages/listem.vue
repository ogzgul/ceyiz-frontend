  ...
  <div class="bg-[#fff5f5] min-h-screen pb-20 font-sans">
      ...
      <div class="max-w-3xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
            ...
            <button @click="toggleNotifications" class="w-10 h-10 rounded-full bg-white text-gray-600 border border-gray-100 hover:text-rose-500 hover:border-rose-100 transition shadow-sm flex items-center justify-center relative">
                ...
                <span v-if="unreadCount > 0" class="absolute top-0 right-0 w-3 h-3 bg-red-500 rounded-full border-2 border-white"></span>
            </button>
    ...
    <div class="max-w-3xl mx-auto px-4 py-6">
      ...
      <div class="space-y-4 min-h-[300px]">
        ...
        <div v-if="loading && (!currentList || currentList.length === 0)" class="flex flex-col items-center justify-center py-12 space-y-4">
        ...
        <div v-else-if="!currentList || currentList.length === 0" class="flex flex-col items-center justify-center py-16 text-center">
        ...
        <div ref="loadMoreTrigger" class="h-10 w-full flex items-center justify-center mt-4">
            ...
            <div v-else-if="!hasMore && currentList && currentList.length > 0" class="text-xs text-gray-300">
...
const currentList = computed(() => {
    const pool = filteredPool.value || []
    return pool.slice(0, displayLimit.value)
})

const hasMore = computed(() => {
    const pool = filteredPool.value || []
    return displayLimit.value < pool.length
})
            ...
            myRecs.forEach((rec: any) => {
                if (rec.liked_by && Array.isArray(rec.liked_by)) {
                    rec.liked_by.forEach((liker: any) => {
                        if (user.value && liker.id !== user.value.id) {
                            tempNotifs.push({ type: 'like', actor: liker.username, message: `"${rec.product?.title || 'paylaşımını'}" beğendi.`, date: rec.updatedAt, read: false })
                        }
                    })
                }
                if (rec.comments && Array.isArray(rec.comments)) {
                    rec.comments.forEach((comment: any) => {
                        if (user.value && comment.users_permissions_user?.id !== user.value.id) {
                            tempNotifs.push({
                                type: 'comment',
    ...
    didOpen: () => {
        ...
        if(fileInput && label) {
            fileInput.addEventListener('change', (e: Event) => { 
                ...
                if (target.files && target.files.length > 0) label.innerHTML = `<i class="fas fa-check text-green-500"></i> ${target.files[0].name}` 
            })
        }
    },
    showCancelButton: true,
        ...
        if(fileInput && urlInput && label) {
            fileInput.addEventListener('change', (e: Event) => { 
                ...
                if (target.files && target.files.length > 0) { 
                    label.innerHTML = `<i class="fas fa-check text-green-500"></i> ${target.files[0].name}`
                    urlInput.value = '' 
                } 
            })
