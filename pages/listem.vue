...
// --- STATE ---
// HATA ÇÖZÜMÜ 1: Varsayılan değer boş dizi olsa bile, sonradan null atanmasını önleyeceğiz.
...
// totalItems template'de kullanılıyorsa hata vermemesi için computed yapıyoruz
...
onMounted(() => {
    ...
    if (cached) {
        try {
            // HATA ÇÖZÜMÜ 2: JSON.parse null dönerse boş dizi ata (|| [])
...
const loadMoreDisplay = () => {
    // HATA ÇÖZÜMÜ 3: filteredPool.value undefined olabilir, kontrol et.
...
const filteredPool = computed(() => {
    // HATA ÇÖZÜMÜ 4: allItems.value undefined ise boş dizi kullan
...
const currentList = computed(() => {
    // HATA ÇÖZÜMÜ 5: filteredPool.value garantisi
    const list = filteredPool.value || []
    return list.slice(0, displayLimit.value)
})

// HATA ÇÖZÜMÜ 6: Length okumadan önce varlık kontrolü (?.) veya default değer (|| 0)
...
// --- HESAPLAMALAR (GÜVENLİ) ---
const safeAllItems = computed(() => allItems.value || [])

const alacaklar = computed(() => safeAllItems.value.filter(i => i && !i.is_purchased)) 
const alinanlar = computed(() => safeAllItems.value.filter(i => i && i.is_purchased))
...
const fetchList = async (reset = true) => {
    ...
    try {
        ...
        // HATA ÇÖZÜMÜ 7: Atama yaparken de array olduğundan emin ol
...
const fetchNotifications = async () => {
    ...
    try {
        ...
        // HATA ÇÖZÜMÜ 8: myRecs'in dizi olduğundan emin ol
...
const deleteItem = async (id: number) => {
  if(confirm('Silinsin mi?')) { 
      // HATA ÇÖZÜMÜ 9: Güvenli filtreleme
